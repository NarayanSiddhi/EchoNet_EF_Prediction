...
class PTEFNetFused(nn.Module):
    def __init__(self, hidden_dim=128, demo_dim=32, backbone: str = "resnet34"):
        super().__init__()

        self.cnn = build_resnet_cnn(backbone)

        self.frame_proj = nn.Linear(512, 128)
        self.fusion_proj = nn.Linear(256, 128)
    ...
    def forward(self, real_video, syn_video, demo_vec):
        ...
        fused_frames = torch.cat([real_feats, syn_feats], dim=2)
        fused_frames = self.fusion_proj(fused_frames)
