...
def main():
    ...
    train_ds = DualVideoEFDataset(
        manifest_path=cfg["data"]["train_manifest_fused"],
    ...
    fused_kw = PTEFNetFused.kwargs_from_cfg(cfg)
    print(f"Fused model kwargs: {fused_kw} | fused_hcl_weight={hcl_w}")
    model = PTEFNetFused(**fused_kw).to(device)
    ...
    for epoch in range(num_epochs):
        ...
        for batch in tqdm(train_loader, desc="Training"):
            ...
            real_video = real_video.to(device, non_blocking=True)
            syn_video = syn_video.to(device, non_blocking=True)
            ...
            with autocast("cuda", enabled=use_amp):
                ef_pred, z = model(real_video, syn_video, demo_vec)
        ...
        if mse < best_mse:
            ...
            torch.save(m_save.state_dict(), best_model_path)
            print(f"✓ Saved best checkpoint (val MSE={mse:.2f}, MAE={mae:.2f})")

        scheduler.step()

    print(f"\n🎉 Run {run_id} done. Best val MSE={best_mse:.2f} (MAE: {best_mae_at_best_mse:.2f})\n")
