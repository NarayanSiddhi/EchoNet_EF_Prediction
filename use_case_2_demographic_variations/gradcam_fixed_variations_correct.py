"""
Correct Grad-CAM Visualizations - Using Better Layer and Proper Upsampling

Uses enc2 (32x32) instead of enc3 (16x16) for better spatial resolution
and proper upsampling to match video frame size.
"""
...
def compute_gradcam(model, video, demographics, target_layer, target_size=(64, 64)):
    """Compute GradCAM with proper upsampling to target size"""
    gradients = []
    activations = []
    ...
    if len(gradients) == 0 or len(activations) == 0:
        ...
        return cam_upsampled.squeeze().detach().cpu().numpy(), output.detach()
    ...
    for t in range(T):
        ...
        cam_normalized.append(cam_t)
    
    cam = torch.stack(cam_normalized, dim=0)  # [T, H, W]
    
    # Upsample to target size using trilinear interpolation
    cam_upsampled = F.interpolate(
        cam.unsqueeze(0).unsqueeze(0),  # [1, 1, T, H, W]
        size=(T, target_size[0], target_size[1]),
        mode='trilinear',
        align_corners=False
    )
    cam_upsampled = cam_upsampled.squeeze(0).squeeze(0)  # [T, H, W]
    
    handle_forward.remove()
    handle_backward.remove()
    
    return cam_upsampled.detach().cpu().numpy(), output.detach()
...
def create_correct_visualization(original_video, original_cam, fixed_video, fixed_cam,
                                 variation_type, output_path, frame_idx=8):
    """Create visualization with correct Grad-CAM"""
...
def analyze_fixed_variations_correct(
    fixed_manifest,
    checkpoint_path=None,
    output_dir='gradcam_fixed_variations_correct',
    device='cuda',
    num_samples=5,
    target_layer_name='enc2'
):
    """Generate correct Grad-CAM visualizations"""
    print("="*70)
    print("GENERATING CORRECT GRADCAM VISUALIZATIONS")
    ...
    # Use enc2 (32x32) instead of enc3 (16x16) for better spatial resolution
    if target_layer_name == 'enc2':
        target_layer = model.enc2
        target_size = (32, 32)  # enc2 output size
    elif target_layer_name == 'enc1':
        target_layer = model.enc1
        target_size = (64, 64)  # enc1 output size (same as input)
    else:
        target_layer = model.enc3
        target_size = (16, 16)  # enc3 output size
    
    fixed_df = pd.read_csv(fixed_manifest)
    sample_ids = fixed_df['original_id'].unique()[:num_samples]
    
    print(f"\nProcessing {len(sample_ids)} samples...")
    print(f"Target layer output size: {target_size}")
    print(f"Upsampling to: (64, 64)")
        ...
        for sample_id in tqdm(sample_ids, desc="Generating"):
            try:
                ...
                with torch.enable_grad():
                    original_cam, _ = compute_gradcam(model, original_video, original_demo, target_layer, target_size=(64, 64))
                ...
                for _, var_row in fixed_samples.iterrows():
                    ...
                    with torch.enable_grad():
                        fixed_cam, _ = compute_gradcam(model, fixed_video, var_demo, target_layer, target_size=(64, 64))
                    
                    sample_dir = output_dir / f"sample_{sample_id:04d}"
                    sample_dir.mkdir(exist_ok=True)
                    vis_path = sample_dir / f"correct_{var_row['variation_type']}.png"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output_dir', type=str, default='gradcam_fixed_variations_correct')
    ...
    analyze_fixed_variations_correct(
